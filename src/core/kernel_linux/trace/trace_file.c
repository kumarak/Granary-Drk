...
trace_file_open(struct inode *inode, struct file *filp)
{
    ...
    {
        //one reader at a time is allowed.
        return -EBUSY;
    }
...
static ssize_t
trace_file_read(struct file *filp, char __user* buf, size_t count, loff_t *f_pos)
{
    ...
    while(trace_file->end == trace_file->current_pos)
    {
        ...
        error = trace_buffer_read_message(trace_file->trace_buffer, trace_process_data,
                                                    !(filp->f_flags & O_NONBLOCK), trace_file);
        if(error < 0)
            return error;
        if(error == 0)
            return -EAGAIN;
...
static unsigned int
trace_file_poll(struct file *filp, poll_table *wait)
{
    ...
    if(!can_read)
    {
        ...
        table.filp = filp;
        table.wait = wait;
        can_read = trace_buffer_poll_read(trace_file->trace_buffer, trace_file_wait_function, &table);
    }
...
void
trace_file_write_message(struct trace_file* trace_file, const void* msg, size_t msg_size)
{
...
void*
trace_file_write_lock(struct trace_file* trace_file, size_t size, void** msg)
{
...
void
trace_file_write_unlock(struct trace_file* trace_file, void* id)
{
 ...
 * Create file in a given directory in debugfs, using which one can
 * read the trace. 'print_message' is interpretator of trace buffer,
 * 'user_data' is the last parameter of this interpretator.
 */
            ...
            snprintf_message print_message, void* user_data)
{
    ...
    if(trace_file == NULL){
        printk("Cannot allocate 'trace_file' structure.");
        return NULL;
    }
    ...
    // Create trace file
    memcpy(&trace_file->trace_file_ops, &trace_file_ops, sizeof(trace_file_ops));
    trace_file->trace_file_ops.owner = m;

    trace_file->file = debugfs_create_file(trace_file_name, S_IRUSR, work_dir, trace_file,
                                                                &trace_file->trace_file_ops);
    if(trace_file->file == NULL)
    {
        printk("Cannot create trace file.");
