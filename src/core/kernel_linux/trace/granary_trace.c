...
#define TRACE_DIR_NAME "granary_tracing"
 ...
 */
struct granary_trace_message
{
    char command[TASK_COMM_LEN];
    pid_t pid;
    granary_trace_pp_function pp;
    char data[0];
};
...
EXPORT_SYMBOL(granary_trace_pp_unregister);
 ...
 * the granary_trace_unlock_commit() for complete trace operation.
 */
void*
granary_trace_lock(granary_trace_pp_function pp, size_t size, void** data)
{
    struct granary_trace_message* msg;
    void* id = trace_file_write_lock(trace_file,
        sizeof(struct granary_trace_message) + size, (void**)&msg);
...
EXPORT_SYMBOL(granary_trace_lock);

/*Complete trace operation, started with granary_trace_lock().*/

void granary_trace_unlock_commit(void* id)
{
    trace_file_write_unlock(trace_file, id);
}
EXPORT_SYMBOL(granary_trace_unlock_commit);
...
void
granary_trace(granary_trace_pp_function pp, const void* data, size_t size)
{
    void* vdata;
    void* id = granary_trace_lock(pp, size, &vdata);
    if(id){
        memcpy(vdata, data, size);
        granary_trace_unlock_commit(id);
    }
}
EXPORT_SYMBOL(granary_trace);
...
int
trace_print_message(char* str, size_t size, const void* msg,
...
int granary_tracer_init(void)
{
    /* create a tracer directory in debugfs for the granary*/
    trace_dir = debugfs_create_dir(trace_dir_name, NULL);
    if(trace_dir == NULL){
        printk("Cannot create trace directory in debugfs.");
        return -EINVAL;
    }

    /* create a tracer file*/
    trace_file = granary_trace_file_create(buffer_size, 1, trace_dir, THIS_MODULE,
    ...
    reset_file = debugfs_create_file(reset_file_name, S_IWUSR | S_IWGRP, trace_dir,
                                                            trace_file, &reset_file_ops);
    if(reset_file == NULL)
    {
        printk("Cannot create reset file.");
        granary_trace_file_destroy(trace_file);
        debugfs_remove(trace_dir);
        return -EINVAL;
    }

    buffer_size_file = debugfs_create_file(buffer_size_file_name, S_IRUGO | S_IWUSR | S_IWGRP,
                                                    trace_dir, trace_file, &buffer_size_file_ops);
    if(buffer_size_file == NULL)
    {
        printk("Cannot create file for control size of buffer.");
    ...
    lost_messages_file = debugfs_create_file(lost_messages_file_name, S_IRUGO,
                                        trace_dir, trace_file, &lost_messages_file_ops);
    if(lost_messages_file == NULL)
    {
        printk("Cannot create file for control number of messages lost.");
...
EXPORT_SYMBOL_GPL(granary_tracer_init);

void granary_tracer_exit(void)
{
...
EXPORT_SYMBOL_GPL(granary_tracer_exit);
    ...
    const void* msg, size_t msg_size, int cpu, u64 ts, void* user_data)
{
    ...
    const struct granary_trace_message* msg_real =
        (const struct granary_trace_message*)msg;
 ...
 * (use only for granary).
 *
 * Implementation.
 */

static int
granary_trace_marker_target_pp_function(char* dest, size_t size,
    const void* data)
{
    bool is_begin;
    ...
    struct module* payload_module, bool is_begin)
{
    ...
    id = granary_trace_lock(granary_trace_marker_target_pp_function,
        size, &data);
...
EXPORT_SYMBOL(granary_trace_marker_target);
