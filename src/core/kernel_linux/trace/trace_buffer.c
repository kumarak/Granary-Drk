...
static int
last_message_array_init_cpu(struct last_message_array* array, int cpu, uint64_t ts)
{
    struct last_message* message = &array->messages[cpu];
    message->msg = NULL;
    message->ts = ts;

    last_message_list_add(&array->messages_ordered, message);
    return 0;
}

static int
last_message_array_init(struct last_message_array* array, uint64_t ts)
{
    int cpu;
...
{
	struct trace_buffer* trace_buffer = kmalloc(sizeof(*trace_buffer), GFP_KERNEL);
    if(trace_buffer == NULL){
        printk("%s: Cannot allocate trace_buffer structure.\n", __FUNCTION__);
        return NULL;
    }

    /*Allocate the ring buffer of given size*/
    trace_buffer->buffer = ring_buffer_alloc(size, mode_overwrite? RB_FL_OVERWRITE : 0);
    if(trace_buffer->buffer == NULL){
        printk("trace_buffer_alloc: Cannot allocate ring buffer.");
        kfree(trace_buffer);
        return NULL;
    }
...
void*
trace_buffer_write_lock(struct trace_buffer* trace_buffer, size_t size, void** msg)
{
    struct trace_data* msg_real;
    struct ring_buffer_event* event = ring_buffer_lock_reserve(trace_buffer->buffer,
