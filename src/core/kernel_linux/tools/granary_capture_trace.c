...
/*need for WEXITSTATUS and WEXITED macros*/
#define _GNU_SOURCE
...
//relative path of trace file in debugfs
#define REL_TRACEFILE "granary/trace"
...
#define PROGRAM_NAME "granary_capture_trace"
...
/* Helper for main */
static size_t
snprintf_trace_filename(char* dest, size_t size, const char* debugfs_mount_point)
{
    return snprintf(dest, size, "%s/%s", debugfs_mount_point, REL_TRACEFILE);
}

int
main(int argc, char* const argv[], char* const envp[]){

    int fd_trace, result = 0;
    enum capture_trace_flags flags;
    const char* debugfs_mount_point;
    ...
    result = process_options(argc, argv, &flags, &debugfs_mount_point,
                                                &file_names, &program_names);

    if(result)
        return result;

    size_t trace_file_name_len = snprintf_trace_filename(NULL, 0, debugfs_mount_point);
    ...
    snprintf_trace_filename(trace_file_name, trace_file_name_len + 1, debugfs_mount_point);
...
static const char**
array_names_add(const char** names, const char* name){
    const char** p = names;
    while(*p) ++p;
    *p = name;
...
static int
process_options(int argc, char* const argv[], enum capture_trace_flags* flags,
                const char** debugfs_mount_point, const char*** file_names, const char*** program_names)
{
    const char **fnames = NULL, **pnames = NULL;
    int opt;
    ...
    *debugfs_mount_point = DEFAULT_DEBUGFS_MOUNT_POINT;
    ...
    if(opt == 'h'){
        printf("Program for capturing trace generated by Granary.\n");
        printf("%s", usage_message);
        return 1;
    }

    fnames = malloc(sizeof(*fnames));
    if(fnames == NULL){
    ...
    for(;opt != -1; opt = getopt_long(argc, argv, short_options, long_options, NULL)){
