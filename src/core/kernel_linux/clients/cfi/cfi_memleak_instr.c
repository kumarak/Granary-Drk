...
static void
instrument_percpu_address(void *drcontext, instrlist_t *ilist, instr_t *instr,
        app_pc pc, struct memory_operand_modifier *ops, bool is_write)
{
    unsigned long used_registers = 0;
    instr_t *emulated;
...
#endif
    PRE(ilist, instr, INSTR_CREATE_mov_imm(drcontext, opnd_unwatched_addr, OPND_CREATE_INT64(0xFFFFFFFFFFFFULL)));
...
noinline void debug_percpu_ptr(instr_t *instr){
    (void)instr;
    printk("%s\n", __FUNCTION__);
}
...
instrument_memory_operations(void *drcontext, void* tag, instrlist_t *ilist, instr_t *instr, app_pc pc, bool is_write)
{
    ...
    if(is_write){
	 ...
	 if(opnd_is_far_base_disp(ops.found_operand)){
	     if(opnd_get_segment(ops.found_operand) == DR_SEG_GS){
	         debug_percpu_ptr(instr);
	     }
	 ...
	 if (opnd_is_rel_addr(ops.found_operand) || opnd_is_abs_addr(ops.found_operand)) {
		 ...
		 if(opnd_is_near_base_disp(ops.found_operand)){
		     ...
		     if(opnd_get_segment(ops.found_operand) == DR_SEG_GS){
		         instrument_percpu_address(drcontext, ilist, instr, pc, &ops, is_write);
		         return ;
		     }
