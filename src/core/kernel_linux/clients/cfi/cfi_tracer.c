        ...
        {   \
            ...
            {   \
                ...
                PRE(ilist, instr, INSTR_CREATE_push(drcontext, opnd_base_addr));    \
                        \
                PRE(ilist, instr, INSTR_CREATE_shl(drcontext, opnd_spill_reg[0],    \
                                OPND_CREATE_INT8(0x3)));    \
                                \
                PRE(ilist, instr, INSTR_CREATE_mov_ld(drcontext, opnd_base_addr,    \
                                        opnd_create_base_disp(reg_descriptor_start, spill_reg[0],   \
                ...
                {   \
                    ...
                    \
                    spill_reg[5] = get_next_free_reg(&used_registers);  \
                    opnd_spill_reg[5] = opnd_create_reg(spill_reg[5]);  \
                    PRE(ilist, instr, INSTR_CREATE_push(drcontext, opnd_spill_reg[5])); \
                    PRE(ilist, instr, INSTR_CREATE_mov_ld(drcontext, opnd_spill_reg[5], opnd_original_addr));    \
                    \
                    PRE(ilist, instr, INSTR_CREATE_shr(drcontext, opnd_spill_reg[5], OPND_CREATE_INT8(0x3)));   \
                    \
                    ...
                    {   \
                        \
                        spill_reg[6] = get_next_free_reg(&used_registers);  \
                        opnd_spill_reg[6] = opnd_create_reg(spill_reg[6]);  \
                        PRE(ilist, instr, INSTR_CREATE_push(drcontext, opnd_spill_reg[6])); \
                        \
                        PRE(ilist, instr, INSTR_CREATE_mov_ld(drcontext, opnd_spill_reg[6], \
                            opnd_create_base_disp(reg_descriptor_start, spill_reg[0],   \
                                    sizeof(reg_t), WRITE_SHADOW_OFFSET, OPSZ_PTR)));    \
                        \
                        PRE(ilist, instr, INSTR_CREATE_bts(drcontext, \
                                opnd_create_base_disp(spill_reg[6], spill_reg[5], \
                                sizeof(reg_t), 0, OPSZ_PTR), opnd_original_addr));   \
                        \
                        PRE(ilist, instr, INSTR_CREATE_pop(drcontext, opnd_spill_reg[6]));  \
                    }   \
                    PRE(ilist, instr, INSTR_CREATE_pop(drcontext, opnd_spill_reg[5]));  \
                }   \
...
tracer_memory_write(void *drcontext, instrlist_t *ilist,
    ...
    PRE(ilist, instr, INSTR_CREATE_cmp(drcontext, opnd_spill_reg[1], opnd_spill_reg[0]));
...
#if 0
    /* update write shadow for watchpoints*/
/*    {
        reg_id_t reg_descriptor_start;
        opnd_t opnd_descriptor_start;
        ...
        PRE(ilist, instr, INSTR_CREATE_shr(drcontext, opnd_spill_reg[0],
                OPND_CREATE_INT8(SHIFT_BIT_COUNT)));

        {
            ...
            PRE(ilist, instr, INSTR_CREATE_sub(drcontext, opnd_spill_reg[1], opnd_base_addr));

            PRE(ilist, instr, INSTR_CREATE_shr(drcontext, opnd_spill_reg[1], OPND_CREATE_INT8(0x3)));

            {
                *//*get bit index for update*//*
                ...
                opnd_spill_reg[5] = opnd_create_reg(spill_reg[5]);
                PRE(ilist, instr, INSTR_CREATE_push(drcontext, opnd_spill_reg[5]));
                PRE(ilist, instr, INSTR_CREATE_mov_ld(drcontext, opnd_spill_reg[5], opnd_spill_reg[1]));

                PRE(ilist, instr, INSTR_CREATE_shr(drcontext, opnd_spill_reg[5], OPND_CREATE_INT8(0x3)));

                PRE(ilist, instr, INSTR_CREATE_and(drcontext, opnd_spill_reg[1],
                        OPND_CREATE_INT32(0x7)));

                {
                    ...
                    PRE(ilist, instr, INSTR_CREATE_bts(drcontext, opnd_create_base_disp(spill_reg[6], spill_reg[5],
                            sizeof(reg_t), 0, OPSZ_PTR), opnd_spill_reg[1]));
    ...
    if(is_write){
     ...
     if (opnd_is_rel_addr(ops.found_operand) || opnd_is_abs_addr(ops.found_operand)) {
         ...
         if(opnd_is_near_base_disp(ops.found_operand)){
             if(is_write){
                 tracer_memory_write(drcontext, ilist, instr, pc, &ops);
             } else {
