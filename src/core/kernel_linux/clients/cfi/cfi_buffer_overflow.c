...
instrument_memory_write(void *drcontext, instrlist_t *ilist,
    ...
    reg_id_t instr_reg;
    opnd_t instr_reg_opnd;
    reg_id_t mask_reg;
    opnd_t mask_reg_opnd;
    opnd_t instr_opnd;

    reg_id_t spill_reg[16];
    opnd_t spill_reg_opnd[16];

    instr_t *addr_watchpoint = INSTR_CREATE_label(drcontext);
    instr_t *addr_not_watchpoint = INSTR_CREATE_label(drcontext);
    ...
    instr_reg = get_next_free_reg(&used_registers);
    instr_reg_opnd = opnd_create_reg(instr_reg);

    mask_reg = get_next_free_reg(&used_registers);
    mask_reg_opnd = opnd_create_reg(mask_reg);

    instr_opnd = ops->found_operand;

    PRE(ilist, instr, begin_instrumenting);

    PRE(ilist, instr, INSTR_CREATE_push(drcontext, instr_reg_opnd));
    PRE(ilist, instr, INSTR_CREATE_push(drcontext, mask_reg_opnd));
    PRE(ilist, instr, INSTR_CREATE_pushf(drcontext));


    PRE(ilist, instr, INSTR_CREATE_lea(drcontext, instr_reg_opnd, opnd_create_base_disp(opnd_get_base(ops->found_operand),
                                                opnd_get_index(ops->found_operand), opnd_get_scale(ops->found_operand),
                                                opnd_get_disp(ops->found_operand), OPSZ_lea)));

    PRE(ilist, instr, INSTR_CREATE_mov_imm(drcontext, mask_reg_opnd,
    									OPND_CREATE_INT64(WATCHPOINT_INDEX_MASK)));

    PRE(ilist, instr, INSTR_CREATE_or(drcontext, mask_reg_opnd, instr_reg_opnd));
    PRE(ilist, instr, INSTR_CREATE_cmp(drcontext, mask_reg_opnd, instr_reg_opnd));

    PRE(ilist, instr, INSTR_CREATE_jcc(drcontext, OP_je, opnd_create_instr(addr_not_watchpoint)));

    /*Address is a watchpoint*/
    PRE(ilist, instr, addr_watchpoint);
    ...
    PRE(ilist, instr, INSTR_CREATE_and(drcontext, spill_reg_opnd[0], instr_reg_opnd));
    PRE(ilist, instr, INSTR_CREATE_shl(drcontext, spill_reg_opnd[0],
                                                   OPND_CREATE_INT8(SHIFT_BIT_COUNT)));

    PRE(ilist, instr, INSTR_CREATE_shr(drcontext, spill_reg_opnd[0],
                                                   OPND_CREATE_INT8(0x1)));
    ...
    PRE(ilist, instr, INSTR_CREATE_cmp(drcontext, mask_reg_opnd,
                        opnd_create_base_disp(spill_reg[1], spill_reg[0],
    ...
    PRE(ilist, instr, INSTR_CREATE_cmp(drcontext, mask_reg_opnd,
    		            opnd_create_base_disp(spill_reg[1], spill_reg[0],
    		                        sizeof(reg_t), 8*sizeof(byte), OPSZ_PTR)));
    ...
    dr_insert_clean_call(drcontext, ilist, instr, watch_buffer_overflow, false, 1,mask_reg_opnd);
    ...
    ops->replacement_operand = opnd_create_base_disp(
            mask_reg, DR_REG_NULL,1, 0 , ops->found_operand.size);
    ...
    PRE(ilist, instr, INSTR_CREATE_pop(drcontext, mask_reg_opnd));
    PRE(ilist, instr, INSTR_CREATE_pop(drcontext, instr_reg_opnd));

    PRE(ilist, instr, INSTR_CREATE_jmp_short(drcontext,
            opnd_create_instr(done_instrumenting)));

    /*if address is not a watchpoint*/
    PRE(ilist, instr, addr_not_watchpoint);
    PRE(ilist, instr, INSTR_CREATE_popf(drcontext));
    PRE(ilist, instr, INSTR_CREATE_pop(drcontext, mask_reg_opnd));
    PRE(ilist, instr, INSTR_CREATE_pop(drcontext, instr_reg_opnd));
...
void
instrument_memory_read(void *drcontext, instrlist_t *ilist, instr_t *instr,
        app_pc pc, struct memory_operand_modifier *ops) {
    ...
    opnd_t instr_opnd = ops->found_operand;
    ...
    PRE(ilist, instr, INSTR_CREATE_lea(drcontext,
                        instr_reg_opnd, opnd_create_base_disp(opnd_get_base(ops->found_operand),
                         opnd_get_index(ops->found_operand), opnd_get_scale(ops->found_operand),
                                                 opnd_get_disp(ops->found_operand), OPSZ_lea)));

    PRE(ilist, instr, INSTR_CREATE_mov_imm(drcontext, mask_reg_opnd, OPND_CREATE_INT64(WATCHPOINT_INDEX_MASK)));
    PRE(ilist, instr, INSTR_CREATE_or(drcontext, mask_reg_opnd, instr_reg_opnd));
    PRE(ilist, instr, INSTR_CREATE_cmp(drcontext, mask_reg_opnd, instr_reg_opnd));
...
void
instrument_rep_movs(void *drcontext, instrlist_t *ilist, instr_t *instr,
        app_pc pc, struct memory_operand_modifier *ops, bool is_write)
{
    unsigned long used_registers = 0;
    reg_id_t instr_reg;
		...
		instrlist_t *ilist, instr_t *instr, app_pc pc, bool is_write)
{
    ...
    if(is_write){
	 ...
	 if(instr->opcode == OP_rep_movs){
	     instrument_rep_movs(drcontext, ilist, instr, pc, &ops, is_write);
	     return;
	 }
...
void
overflow_instrument_write(void *drcontext, instrlist_t *ilist, instr_t *instr, app_pc pc, struct memory_operand_modifier *ops)
{
    unsigned long used_registers = 0;
    bool flag_unwatched_addr = false;
    reg_id_t instr_reg;
    ...
    instr_reg = get_next_free_reg(&used_registers);
    opnd_t opnd_instr_reg = opnd_create_reg(instr_reg);

    mask_reg = get_next_free_reg(&used_registers);
    opnd_t opnd_mask_reg = opnd_create_reg(mask_reg);
    ...
    PRE(ilist, instr, begin_instrumenting);
    PRE(ilist, instr, INSTR_CREATE_push(drcontext, opnd_instr_reg));
    PRE(ilist, instr, INSTR_CREATE_push(drcontext, opnd_mask_reg));
    PRE(ilist, instr, INSTR_CREATE_pushf(drcontext));


    PRE(ilist, instr, INSTR_CREATE_lea(drcontext, opnd_instr_reg, opnd_create_base_disp(opnd_get_base(ops->found_operand),
                                                opnd_get_index(ops->found_operand), opnd_get_scale(ops->found_operand),
                                                opnd_get_disp(ops->found_operand), OPSZ_lea)));

    PRE(ilist, instr, INSTR_CREATE_mov_imm(drcontext, opnd_mask_reg, OPND_CREATE_INT64(WATCHPOINT_INDEX_MASK)));

    PRE(ilist, instr, INSTR_CREATE_or(drcontext, opnd_mask_reg, opnd_instr_reg));
    PRE(ilist, instr, INSTR_CREATE_cmp(drcontext, opnd_mask_reg, opnd_instr_reg));
    ...
    /*get the descriptor table index*/
    PRE(ilist, instr, INSTR_CREATE_and(drcontext, spill_reg_opnd[0], opnd_instr_reg));
    PRE(ilist, instr, INSTR_CREATE_shr(drcontext, spill_reg_opnd[0],
                                                   OPND_CREATE_INT8(SHIFT_BIT_COUNT)));
    ...
    PRE(ilist, instr, INSTR_CREATE_cmp(drcontext, opnd_mask_reg,
                        opnd_create_base_disp(spill_reg[1], spill_reg[0],
                                               sizeof(reg_t/*struct alias_meta*/), 0, OPSZ_PTR)));

    PRE(ilist, instr, INSTR_CREATE_jcc(drcontext, OP_jl, opnd_create_instr(do_callback)));
    ...
    dr_insert_clean_call(drcontext, ilist, instr, watch_buffer_overflow, false, 1, opnd_mask_reg);
    ...
    PRE(ilist, instr, INSTR_CREATE_popf(drcontext));
    PRE(ilist, instr, emulated);
    PRE(ilist, instr, INSTR_CREATE_pop(drcontext, opnd_mask_reg));
    PRE(ilist, instr, INSTR_CREATE_pop(drcontext, opnd_instr_reg));
        ...
        instr_t *instr, app_pc pc, bool flag)
{
    ...
    if (opnd_is_rel_addr(instr_opnd) || opnd_is_abs_addr(instr_opnd)) {
        ...
        PRE(ilist, instr, INSTR_CREATE_pop(drcontext, opnd_reg_mask));
        PRE(ilist, instr, INSTR_CREATE_pop(drcontext, opnd_reg));
        PRE(ilist, instr, INSTR_CREATE_jmp(drcontext,
                opnd_create_instr(done_instrumenting)));
...
instrument_memory_operations(void *drcontext, void* tag, instrlist_t *ilist, instr_t *instr, app_pc pc, bool is_write)
{
    ...
    if(is_write){
     ...
     if (opnd_is_rel_addr(ops.found_operand) || opnd_is_abs_addr(ops.found_operand)) {
         ...
         if(opnd_is_near_base_disp(ops.found_operand)){
             if(is_write){
                 overflow_instrument_write(drcontext, ilist, instr, pc, &ops);
             } else {
                 instrument_memory_read(drcontext, ilist, instr, pc, &ops);
             }
         }else if(opnd_is_far_base_disp(ops.found_operand)) {
