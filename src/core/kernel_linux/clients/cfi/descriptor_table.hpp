...
#endif
#ifdef CONFIG_TRACER
            descriptors->leak_descriptor->type_id = 0;
            descriptors->leak_descriptor->shadow_size = (size/8);
            descriptors->leak_descriptor->write_shadow = (uint64_t)cfi_kmalloc(descriptors->leak_descriptor->shadow_size, GFP_ATOMIC);
            memset((void*)descriptors->leak_descriptor->write_shadow, 0x0, descriptors->leak_descriptor->shadow_size);

            descriptors->leak_descriptor->read_shadow = (uint64_t)cfi_kmalloc(descriptors->leak_descriptor->shadow_size, GFP_ATOMIC);
            memset((void*)descriptors->leak_descriptor->read_shadow, 0x0, descriptors->leak_descriptor->shadow_size);
#endif
