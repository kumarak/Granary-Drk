...
def pre_wrap_var(ctype, var_name, O, indent="        "):
  ...
  if not var_name:
    ...
    O(indent, "WRAP_RECURSIVE(", var_name, ");")

def pre_wrap_trace(ctype, var_name, O, indent="        "):
  intern_ctype = ctype.base_type()
  if not must_wrap([intern_ctype]):
    return
  if not var_name:
    ...
    O(indent, "TRACES_WATCHPOINT(", var_name, ");")
...
def wrap_function(ctype, orig_ctype, func):
  ...
  for (arg_ctype, arg_name) in zip(ctype.param_types, param_names):
    ...
    pre_wrap_trace(arg_ctype, arg_name, O, indent="    ")
    #O("    TRACES_WATCHPOINT(", arg_name, ");")
