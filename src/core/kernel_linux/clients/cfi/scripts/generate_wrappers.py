...
def will_pre_wrap_fileds(ctype):
  ...
  for ctype, field_name in ctype.fields():
    ...
    if not field_name:
      ...
      or isinstance(intern_ctype, CTypeUnion):
...
#  O("    NO_RETURN")
  ...
  O("    MODULE_PRE {")
  O("		D( kern_printk(\"", name, "\");)")
  O("		ADD_TO_HASH(arg, SCAN_FUNCTION(", name, "))")
  O("		ADD_TYPELESS_WATCHPOINT(arg);")
 # pre_wrap_fields(ctype, O)
  O("    }")
  O("    MODULE_POST {")
  O("		D( kern_printk(\"", name, "\");)")
#  O("		ADD_TO_HASH(arg, SCAN_FUNCTION(", name, "))")
  O("		REMOVE_TYPELESS_WATCHPOINT(arg);")
 # pre_wrap_fields(ctype, O)
  O("    }")
  O("    NO_MODULE_RETURN")
  O("})")
