...
void cfi_try_wait_on_module(void) {
    volatile int prev_blocked;

    struct cfi_client_extension *cfi = (struct cfi_client_extension *)dr_get_client_extension();

    // we're in the context of the current running module thread, so we're
    // allowed to continue.
    if(cfi->can_continue) {
        return;
    }
#if 1
    // atomically swap blocked with 1; if the prev_blocked != blocked, then
    // that means prev_blocked = 0 and this guy can move forward.
    for(;;) {
        __asm__ __volatile__ (
            ...
            "mov %1, %%rax;"
        "1:"
            "mov %%rax, %0;"            // prev_blocked = blocked
            "mov $0x1, %%rdx;"          // new_blocked = 1

            "lock;"
            "cmpxchg %%rdx, %1;"        // if current(blocked(%0))
                                        // == recorded(blocked(%rax))
                                        //    then new(blocked(%1)) = block(%rdx), ZF = 1
                                        //    else recorded(blocked(%rax)) = current(blocked(%0)), ZF = 0
            "jnz 1b;"
...
#endif
    //printk("entering module context...\n");

    cfi->can_continue = 1;
}

void cfi_release_module(void) {
    volatile int prev_blocked;

    //printk("   ... exiting module context\n");
    ...
    //printk("   ... released blocking lock = %d\n", prev_blocked);

    BUG_ON(1 != prev_blocked);
}
